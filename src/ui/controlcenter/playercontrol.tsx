...
export class PlayerControl extends ControlWidget {
  ...
  private updateNoMusicPlaying(): void {
    (this.currentRender.get_children_by_id("metadata")[0] as TextBox).markup = `<span style='italic' weight='light' color='${theme.text.subdued}'>No music playing</span>`;
  }

  private updatePlaybackStatus(playbackStatus: MediaPlayer2Changed['PlaybackStatus']): void {
    this.playbackStatus = playbackStatus;
    (this.currentRender.get_children_by_id("play")[0] as Imagebox).image = gears.color.recolor_image(playbackStatus === "Playing" ? PAUSE_ICON : PLAY_ICON, theme.text.subdued);
  ...
  override onInit(): void {
    ...
    dbus.session().onSignal<[string, MediaPlayer2Changed]>(null, "org.freedesktop.DBus.Properties", "PropertiesChanged", "/org/mpris/MediaPlayer2", (event) => {
    ...
    dbus.session().getProperty<string>("org.mpris.MediaPlayer2.playerctld", "/org/mpris/MediaPlayer2", "org.mpris.MediaPlayer2.Player", "PlaybackStatus").then((playbackStatus) => {
      this.updatePlaybackStatus(playbackStatus as MediaPlayer2Changed['PlaybackStatus']);
    });

    dbus.session().getProperty<boolean>("org.mpris.MediaPlayer2.playerctld", "/org/mpris/MediaPlayer2", "org.mpris.MediaPlayer2.Player", "CanPlay").then((canPlay) => {
      this.updateCanPlay(canPlay);
    });
    
    dbus.session().getProperty<boolean>("org.mpris.MediaPlayer2.playerctld", "/org/mpris/MediaPlayer2", "org.mpris.MediaPlayer2.Player", "CanGoNext").then((canGoNext) => {
      this.updateCanGoNext(canGoNext);
    });

    dbus.session().getProperty<MediaPlayer2Changed['Metadata']>("org.mpris.MediaPlayer2.playerctld", "/org/mpris/MediaPlayer2", "org.mpris.MediaPlayer2.Player", "Metadata").then((metadata) => {
      if (metadata) {
        ...
        <wibox.container.margin margins={dpi(8, s)}>
          <wibox.layout.align.horizontal spacing={dpi(8, s)}>
            ...
            <wibox.layout.fixed.horizontal spacing={dpi(8, s)}>
              <wibox.widget.imagebox id="play" image={gears.color.recolor_image(PLAY_ICON, theme.text.subdued)} buttons={awful.button([], MouseButton.PRIMARY, () => this.playPause())} forced_height={dpi(20, s)} visible={false} />
              <wibox.widget.imagebox id="next" image={gears.color.recolor_image(`${ICON_PATH}/player-next.png`, theme.text.subdued)} buttons={awful.button([], MouseButton.PRIMARY, () => this.next())} forced_height={dpi(20, s)} visible={false} />
