...
import { ControlWidget, Handler } from "./control-widget";
...
import { execute, spawn, spawnOnce } from "../../lib/process";
...
const lightIcon = `${ICON_PATH}/light.png`;
...
export class SettingsWidget extends ControlWidget {
  ...
  private elgatoKeyLightIp: string | null;

  constructor(handler: Handler) {
    super(handler);
    this.elgatoKeyLightIp = config("elgato_key_light_ip", null);
  }
  ...
  private onDndToggle() {
    ...
    (this.currentRender.get_children_by_id("dnd")[0] as BackgroundContainer).bg = dnd
      ? theme.controlcenter.settings.dnd
      : theme.controlcenter.settings.disabled;
  ...
  private async toggleLight(): Promise<void> {
    const { stdout: status } = await execute(`${SCRIPT_PATH}/elgato-key-light.sh ${this.elgatoKeyLightIp}`);
    this.onLightToggle(status?.trim() === "1");
  }

  private onLightToggle(status: boolean): void {
    (this.currentRender.get_children_by_id("light")[0] as BackgroundContainer).bg = status
      ? theme.controlcenter.settings.light
      : theme.controlcenter.settings.disabled;
    (this.currentRender.get_children_by_id("light-icon")[0] as Imagebox).image = status
      ? gears.color.recolor_image(lightIcon, theme.controlcenter.settings.icon.active)
      : gears.color.recolor_image(lightIcon, theme.controlcenter.settings.icon.disabled);
  }
  ...
  private renderSetting(s: Screen, id: string, icon: string, onClick: () => void) {
    ...
    return (
      <wibox.container.background
        ...
        bg={theme.controlcenter.settings.disabled}
  ...
  private updateBatteryWidget() {
    dbus
      .system()
      .call("org.freedesktop.UPower", "/org/freedesktop/UPower/devices/DisplayDevice", "org.freedesktop.DBus.Properties", "GetAll", [
      ...
      .then((response) => {
        ...
        if (
          ...
          status.State === BatteryState.FullyCharged
        ) {
  ...
  override onInit(): void {
    ...
    dbus
      .system()
      .onSignal(null, "org.freedesktop.DBus.Properties", "PropertiesChanged", "/org/freedesktop/UPower/devices/DisplayDevice", (signal) => {
      ...
      <wibox.container.margin right={dpi(4, s)} left={dpi(4, s)}>
        <wibox.layout.align.horizontal halign="right">
          ...
          <wibox.layout.fixed.horizontal spacing={dpi(8, s)}>
            {this.elgatoKeyLightIp &&
              this.renderSetting(s, "light", lightIcon, () => {
                this.toggleLight();
              })}
            {this.renderSetting(s, "sleep", sleepIcon, () => {
              this.toggleSleep();
            })}
            {this.renderSetting(s, "dnd", dndIcon, () => {
              this.toggleDnd();
            })}
          </wibox.layout.fixed.horizontal>
