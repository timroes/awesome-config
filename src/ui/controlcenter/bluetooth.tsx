...
const ICON_MAP: Record<string, string> = {
  ...
  "audio-headphones": "audio-headset.png",
  "audio-apple": "device-airpods.png",
  "audio-card": "music.png",
};

const DEFAULT_ICON = "bluetooth.png";

const isApple = (address: string): boolean => {
  return address.startsWith("EC:46:54");
};
  ...
  private onUpdate() {
    ...
    for (const [path, info] of Object.entries(this.devices)) {
      const buttons = awful.button([], 1, () => {
        ...
        dbus
          .system()
          .call("org.bluez", path, "org.bluez.Device1", info.connected ? "Disconnect" : "Connect", [])
          .catch(() => {
          ...
          <wibox.container.margin margins={dpi(12, this.screen)} buttons={buttons}>
            <wibox.layout.fixed.horizontal spacing={dpi(6, this.screen)}>
              <wibox.widget.imagebox
                image={gears.color.recolor_image(`${ICON_PATH}/${icon}`, color)}
                forced_height={dpi(14, this.screen)}
              />
    ...
    dbus
      ...
      .then((result) => {
        ...
        for (const [path, info] of r.pairs()) {
          log(`Bluetooth device found: ${inspect(info["org.bluez.Device1"])}`);
          if (info["org.bluez.Device1"]) {
            this.devices[path] = {
              name: info["org.bluez.Device1"]["Alias"] || info["org.bluez.Device1"]["Name"],
              icon: isApple(info["org.bluez.Device1"]["Address"]) ? "audio-apple" : info["org.bluez.Device1"]["Icon"],
    ...
    return (
      <wibox.container.background bg={theme.controlcenter.panel} shape={gears.shape.rounded_rect}>
        <wibox.layout.fixed.vertical
          id="devices"
          spacing={dpi(2, s)}
          spacing_widget={<wibox.widget.separator span_ratio={0.95} color={theme.controlcenter.backround} />}
        />
