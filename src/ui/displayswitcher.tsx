...
import { ICON_PATH, SUPER, XProperties } from "../lib/constants";
...
const screenLayouts: ScreenLayout[] = [
  {
    ...
    fn: async (screens: ScreenInfo[]) => {
      ...
      if (internal) {
        await execute(
          `xrandr --output ${internal.id} --auto --primary ${screens
            .filter((s) => !s.internal)
            .map((s) => `--output ${s.id} --off`)
            .join(" ")}`
        );
      }
    },
  },
  {
    ...
    fn: async (screens: ScreenInfo[]) => {
      ...
      if (connectedExternals.length > 0) {
        await execute(
          `xrandr --output ${connectedExternals[0].id} --auto --primary ${connectedExternals
            .slice(1)
            .map((s) => `--left-of ${s.id} --output ${s.id} --auto`)
            .join(" ")} ${screens
            .filter((s) => !connectedExternals.includes(s))
            .map((s) => `--output ${s.id} --off`)
            .join(" ")}`
        );
      }
  ...
  },
  {
    ...
    fn: async (screens: ScreenInfo[]) => {
      ...
      if (allConnected.length > 0) {
        await execute(
          `xrandr --output ${allConnected[0].id} --auto --primary ${allConnected
            .slice(1)
            .map((s) => `--left-of ${s.id} --output ${s.id} --auto`)
            .join(" ")} ${screens
...
async function getScreenInfos(): Promise<ScreenInfo[]> {
  ...
  return (
    output
      ?.split("\n")
      .filter((line) => line.includes("normal"))
      .map((line) => {
...
function createLayoutItem(s: Screen, layout: ScreenLayout, focused: boolean, disabled: boolean) {
  const laptopImage = gears.color.recolor_image(
    `${ICON_PATH}/display-internal-${layout.icons.internal ? "enabled" : "disabled"}.png`,
    focused ? theme.text.dark : theme.text.light
  );
  const externalImage = gears.color.recolor_image(
    `${ICON_PATH}/display-external-${layout.icons.external ? "enabled" : "disabled"}.png`,
    focused ? theme.text.dark : theme.text.light
  );
  return (
    <wibox.container.background
      bg={focused ? theme.highlight.regular : theme.bg.panel}
      fg={focused ? theme.text.dark : theme.text.light}
    ...
    root_keybindings: [
      [
        ...
        async () => {
          ...
          switcher.set_xproperty(XProperties.ANIMATIONS.POP_IN_SHORT, true);
        },
      ],
    ],
