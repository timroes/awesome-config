...
function naughty.notify(args)
    ...
    local actionslayout = wibox.container.margin()
    local actions_max_width = 0
    local actions_total_height = 0
		local actionmargin = margin * 2.5
    if actions then
			actionslayout.top = actionmargin
			actions_total_height = actionmargin
        local actionslist = wibox.layout.fixed.vertical()
				actionslayout.widget = actionslist
        for action, callback in pairs(actions) do
            ...
            actionmarginbox:set_margins(actionmargin)
            actionmarginbox:set_widget(actiontextbox)
            actiontextbox:set_valign("middle")
            actiontextbox:set_font(font)
            actiontextbox:set_text(action)
            -- calculate the height and width
            local w, h = actiontextbox:get_preferred_size(s)
            local action_height = h + 2 * actionmargin
            local action_width = w + 2 * actionmargin

            actionmarginbox:buttons(gtable.join(
                button({ }, 1, callback),
                button({ }, 3, callback)
                ))
						local actionbg = wibox.widget.background(actionmarginbox, nil)
						actionbg:connect_signal('mouse::enter', function()
							actionbg.bg = '#E0E0E0'
						end)
						actionbg:connect_signal('mouse::leave', function()
							actionbg.bg = nil
						end)
            local actionborderbox = wibox.widget {
							actionbg,
							top = 1,
							color = '#DDDDDD',
							widget = wibox.container.margin
						}
            actionslist:add(actionborderbox)
